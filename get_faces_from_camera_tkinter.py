    ...
    def GUI_info(self):
        ...
        self.folder_combobox.grid(row=6, column=1, sticky=tk.W, padx=0, pady=2)
    ...
    def get_frame(self):
        try:
            if self.cap.isOpened():
                ...
                return ret, cv2.cvtColor(self.imgBackground, cv2.COLOR_BGR2RGB)
    ...
    def process(self):
        if keyboard.is_pressed('Esc'):
            self.win.destroy()  # Close the Tkinter window if 'q' is pressed
            return
        ...
        if ret:
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    if (d.right() + self.ww) > 55+640 or (d.bottom() + self.hh > 162+480) or (d.left() - self.ww < 0) or (
                            d.top() - self.hh < 0):
